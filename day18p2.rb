...
class WompNumber < Numeric
  ...
  def +(other)
    WompNumber.new(value * other.value)
  end
  ...
  def self.eval(expr)
    expr = expr
             .gsub(/(\d+)/) { |x| "WompNumber.new(#{x})" }
             .gsub(/([+*])/) { |ch| ch == '*' ? '+' : '*' }
    Kernel.eval(expr).to_i
  end
end

puts INPUT_LINES.map { WompNumber.eval(line) }.reduce{|a, b| a + b}
...
puts lines.map{|line| WompNumber.eval(line)}.reduce{|a, b| a + b }
